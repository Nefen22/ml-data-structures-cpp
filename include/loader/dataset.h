...
public:
    ...
    */
    int len(){
         ...
         */
        return this->data_shape[0]; //remove it when complete
    }
    ...
    DataLabel<DType, LType> getitem(int index){
        ...
        if (this->label.dimension() > 0) reLabel = xt::view(label, index);
        return DataLabel<DType, LType>(xt::view(data, index), reLabel);
    }
