...
double_tensor calc_classifcation_metrics(ulong_tensor y_true, ulong_tensor y_pred, int nclasses) {
    ...
    double eps = 1e-7; // Giá trị chống chia cho 0

    ulong_tensor C = confusion_matrix(y_true, y_pred, nclasses);
    ulong nsamples = xt::sum(C)[0];
    if (nsamples == 0) return lookup; // Tránh chia cho 0 nếu tập dữ liệu rỗng
    ...
    // SỬA TẠI ĐÂY: Thêm eps vào mẫu số
    double_tensor prec_per_class = diag / (pred_per_class + eps);
    double_tensor recall_per_class = diag / (label_per_class + eps);
    double_tensor weights = label_per_class / (double(nsamples) + eps);
    double_tensor f1_measures = (2 * diag) / (pred_per_class + label_per_class + eps);

    lookup[ulong(ACCURACY)] = double(ncorrect) / double(nsamples);
